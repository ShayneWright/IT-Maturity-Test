...
export function Results({result,contact}:{result:Result;contact:Contact}){
  ...
  return <main className="results">
    ...
    <section className="report-grid">
      <article className="chart-card"><div className="card-heading"><div><span className="overline">READINESS BY AREA</span><h2>Your security profile</h2></div><span className="benchmark"><i/>Target: 3.0</span></div><div className="chart"><ResponsiveContainer width="100%" height="100%"><RadarChart data={chart} outerRadius="68%"><PolarGrid stroke="#d6dfeb"/><PolarAngleAxis dataKey="subject" tick={{fill:'#42536a',fontSize:12}}/><PolarRadiusAxis domain={[0,5]} tick={false} axisLine={false}/><Radar name="Your score" dataKey="score" stroke="#1976d2" fill="#25a4e8" fillOpacity={.28} strokeWidth={3}/><Radar name="Target" dataKey="target" stroke="#90a1b5" fill="transparent" strokeDasharray="5 5"/><Legend/></RadarChart></ResponsiveContainer></div><p className="sr-only">Scores out of five: {chart.map(item=>`${item.subject} ${item.score}`).join(', ')}.</p></article>
